...
7. `product`
8. `product_variant`
9. `product_composition` e `product_composition_set` (mesma prioridade de ordenação interna)
10. `inventory_movement`
11. `order`
12. `purchase_order`
13. `purchase_receipt`
14. `product_cost_snapshot`
...
**Tamanho do chunk (Worker):** variável opcional **`INGESTION_QUEUE_CHUNK_SIZE`** (objectos por invocação; por defeito **5**, teto **200** no código). Valores mais baixos usam menos CPU por mensagem; nas continuações o Worker **não** volta a correr Zod/validação semântica completa no payload inteiro (só no 1.º chunk). No **Workers Free** o teto de CPU por invocação é baixo — usa chunk **1–5** se vires `exceededCpu`; em **Workers Paid** podes definir `[limits] cpu_ms` no `wrangler.toml` ou no dashboard.

**Budget de imagens por execução:** variável opcional **`INGESTION_QUEUE_MAX_IMAGE_URLS_PER_RUN`** (main + galeria; por defeito **12**). Quando o budget acaba, o objecto segue sem upload de imagens por URL no chunk actual e recebe warning explícito para reprocessar imagens em chunk seguinte/lote dedicado.

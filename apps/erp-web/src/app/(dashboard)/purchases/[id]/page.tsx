    ...
    <div className="flex flex-col overflow-auto">
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        <div className="flex items-center gap-3">
          <Link href="/purchases" className="text-sm text-zinc-600 hover:text-zinc-900">← Compras</Link>
              ...
              <tbody>
                {order.items.map((item) => (
                  <tr key={item.id} className="border-b border-zinc-50 last:border-0">
                    ...
                    <td className="py-2 pr-4 text-right tabular-nums text-zinc-600">{item.quantity}</td>
                    <td className="py-2 pr-4 text-right tabular-nums">
                      <span className={item.receivedQuantity >= item.quantity ? "text-green-600" : "text-zinc-600"}>
                        {item.receivedQuantity}
                      </span>
                    </td>
                    <td className="py-2 pr-4 text-right tabular-nums text-zinc-600">{formatCurrency(item.unitPriceCents)}</td>
                    <td className="py-2 text-right tabular-nums font-medium">{formatCurrency(item.totalPriceCents)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t border-zinc-200">
                  <td colSpan={4} className="pt-3 text-right text-sm font-semibold text-zinc-700">Total</td>
                  <td className="pt-3 text-right tabular-nums font-bold text-zinc-900">{formatCurrency(order.totalAmountCents)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          <div className="space-y-4">
            ...
            {order.status === "draft" && (
              <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                ...
                <form action={updatePurchaseStatusAction} className="space-y-2">
                  ...
                  <button type="submit" className="w-full rounded-md bg-blue-600 py-2 text-sm font-medium text-white hover:bg-blue-700">
                    Confirmar pedido
                  </button>
                </form>
              </div>
            ...
            {canReceive && (
              <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                ...
                <form action={receivePurchaseOrderAction} className="space-y-3 text-sm">
                  ...
                  {order.items.filter((i) => i.receivedQuantity < i.quantity).map((item) => (
                    <div key={item.id} className="rounded-md bg-zinc-50 p-3">
                      <p className="mb-1 font-medium text-zinc-700">{item.description}</p>
                      <p className="mb-2 text-xs text-zinc-500">Pendente: {item.quantity - item.receivedQuantity} de {item.quantity}</p>
                  ...
                  <button type="submit" className="w-full rounded-md bg-green-600 py-2 text-sm font-medium text-white hover:bg-green-700">
            ...
            {canCancel && (
              <form action={updatePurchaseStatusAction}>
                ...
                <button type="submit" className="w-full rounded-md border border-red-200 py-2 text-sm font-medium text-red-600 hover:bg-red-50">
                  Cancelar ordem
                </button>
              </form>
            )}

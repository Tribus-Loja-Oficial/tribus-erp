...
export default async function ReceivablesPage({
  ...
  try {
    const [r, c, a] = await Promise.all([
      erpApiFetch<{ data: Receivable[] }>({ path: "/finance/receivables", searchParams: { limit: 40 } }),
    ...
    <div className="flex flex-col overflow-auto">
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        ...
        <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-semibold text-zinc-800">Nova conta a receber</h2>
                ...
                rows.map((r) => (
                  <tr key={r.id} className="border-b border-zinc-100 last:border-0">
                    ...
                    <td className="px-4 py-3">
                      {r.status !== "received" && r.status !== "cancelled" && accounts.length > 0 && (
                        <form action={receiveReceivableAction} className="flex items-center gap-1">
